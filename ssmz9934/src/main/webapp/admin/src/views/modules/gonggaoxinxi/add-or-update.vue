<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="公告分类" prop="gonggaofenlei">
          <el-select :disabled="ro.gonggaofenlei" v-model="ruleForm.gonggaofenlei" placeholder="请选择公告分类">
            <el-option
                v-for="(item,index) in gonggaofenleiOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/gonggaofenlei/gonggaofenlei`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.gonggaofenleiOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../ssmz9934/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
